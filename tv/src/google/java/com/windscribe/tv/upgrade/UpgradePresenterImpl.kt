...
class UpgradePresenterImpl
    ...
    constructor(
        ...
        private fun handleAmazonReceipt(
            ...
            receipt.productType?.let {
                when (it) {
                    ...
                    ProductType.CONSUMABLE, ProductType.SUBSCRIPTION -> {
        ...
        override fun onSkuDetailsReceived(
            responseCode: Int,
        ...
        private fun getBillingErrorMessage(responseCode: Int): String {
            when (responseCode) {
                BillingResponseCode.BILLING_UNAVAILABLE -> {
                    ...
                    return appContext
                        .resources
                        .getString(com.windscribe.vpn.R.string.billing_unavailable)
                }
                ...
                BillingResponseCode.ITEM_UNAVAILABLE -> {
                    ...
                    return appContext
                        .resources
                        .getString(com.windscribe.vpn.R.string.item_unavailable)
                }
                ...
                BillingResponseCode.SERVICE_UNAVAILABLE -> {
                    ...
                    return appContext
                        .resources
                        .getString(com.windscribe.vpn.R.string.billing_service_unavailable)
                }
 ...
 User will be shown unknown error. Support please look for the token in the log. Response code: $responseCode""",
                ...
                if (upgradeView.billingType === UpgradeActivity.BillingType.Amazon) {
                    ...
                    for (sku in skuList) {
                        val planType =
                            mobileBillingPlans
                                .stream()
                                .filter { billingPlans: BillingPlans ->
        ...
        /** Persist the latest account status to the local DB (TV-specific). Best-effort. */
        private suspend fun updateUserStatus() {
            val user = userRepository.user.value ?: return
            try {
                localDbInterface.updateUserStatus(
                    UserStatusTable(user.userName, user.userStatusInt, user.accountStatusToInt),
                )
            } catch (throwable: Throwable) {
                logger.debug("Error updating user status table. " + instance.convertThrowableToString(throwable))
            }
        }
