...
class WelcomePresenterImpl
    ...
    constructor(
        ...
        override fun onAuthSignUpClick(
            ...
            if (validateLoginInputs(username, password, email ?: "", false)) {
                ...
                activityScope.launch(Dispatchers.IO) {
                    ...
                    val result =
                        result<AuthToken> {
                            apiCallManager.authTokenSignup(username, true)
                        }
                    withContext(Dispatchers.Main) {
                        when (result) {
                            is CallResult.Success -> {
        ...
        private fun validateUsernameCharacters(username: String): Boolean = Pattern.matches("[a-zA-Z0-9_-]*", username)
    }
