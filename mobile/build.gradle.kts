...
plugins {
    ...
    // Consumes the generated ART profile from :baselineprofile and packages it.
...
// The androidx.baselineprofile plugin derives `nonMinifiedRelease` and
// `benchmarkRelease` build types from `release`, which inherits
// signingConfig = signingConfigs.release (config/config.gradle). That keystore
// only exists when config/sign.properties is present (CI), so local profile
// generation fails packaging with "SigningConfig release is missing storeFile".
// These are generation-only variants and don't need release signing — re-point
// them at the debug signing config via the Variant API so generation works
...
dependencies {
    ...
    // profileinstaller applies the profile on-device; the profile itself is
    // generated by the :baselineprofile module (real login -> Home journey) and
    // wired in via the baselineProfile() dependency below.
